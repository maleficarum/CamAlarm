...
/**
 * Skype key pair manager
 * 
 * @author Oscar Ivan Hernandez Ventura [ oscar at angellore dot mx]
 *
 * @version $ Id : SkypeKeyPairManager.java , v 1.0 19/09/2011 angellore $
 */
public class SkypeKeyPairManager {
	...
	public boolean readAppKeyPairFromFile(String pathName) {
		...
		if (i >= j) {
			if (pathName.charAt((pathName.length() - 3)) == '.') {
				...
				logger.error("readAppKeyPairFromFile: Unrecognized Certificate file suffix. {}", pathName);
				j = KEY_PAIR_FILE_SUFFIXES.length;
				for (i = 0; i < j; i++) {
					logger.info("{}", KEY_PAIR_FILE_SUFFIXES[i]);
				}
				return (false);
			} else {
		...
		try {
			...
			logger.error("readAppKeyPairFromFile: Could not find certificate file: {}", fullPathName);
			return (false);
		} catch (IOException e2) {
			logger.error("readAppKeyPairFromFile: Unable to read certificate file:{}", fullPathName);
			return (false);
		} catch (InvalidKeySpecException e3) {
			logger.error("readAppKeyPairFromFile: Certificate file {} contains invalid certficate data.",fullPathName);
			return(false);
		}		
